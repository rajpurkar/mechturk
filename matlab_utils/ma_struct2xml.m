  ...
  for i = 1:length(names)
    ...
    elseif isstruct(s.(names{i}))
      for j = 1:length(s.(names{i}))
        res = [res tagged_string(ma_struct2xml(s.(names{i})(j)), names{i}), nl_char];
      end
    elseif islogical(s.(names{i}))
      str_val = '0';
      if s.(names{i})
        str_val = '1';
      end
        
      res = [res tagged_string(str_val, names{i}) nl_char];
    else
