...
function annolist = amt_to_al(basedir, amt_results)
  ...
  for aidx = 2:size(A, 1)
	...
	if ~isempty(strfind(tokens{1}, '.jpeg')) 
      ...
      [fpath1, fname1, fext1] = splitpathext(tokens{1});
	  ...
	  assert(mod(length(tokens) - 1, 6) == 0);
	  ...
	  for hit_ridx = 1:num_rects
	    ...
	    annolist(hit_aidx).annorect(hit_ridx).x1 = round(str2num(tokens{6*(hit_ridx - 1) + 2}));
	    annolist(hit_aidx).annorect(hit_ridx).y1 = round(str2num(tokens{6*(hit_ridx - 1) + 3}));
	    annolist(hit_aidx).annorect(hit_ridx).x2 = round(str2num(tokens{6*(hit_ridx - 1) + 4}));
	    annolist(hit_aidx).annorect(hit_ridx).y2 = round(str2num(tokens{6*(hit_ridx - 1) + 5}));
